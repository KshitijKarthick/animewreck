...
from server.constants import lstm_hidden_dim, lstm_layers, bidirectional, num_anime, device, \
    pretrained_learner_fname, pretrained_anime_genre_embeddings_file, past_anime_length, \
...
class Net(nn.Module):
    ...
    def __init__(self, anime_embedding_vocab, anime_embedding_dim,
        ...
        self.lstm = nn.LSTM(anime_embedding_dim + 11, lstm_hidden_dim,
                            lstm_layers, bidirectional=bidirectional)
        self.fc1 = nn.Linear(lstm_hidden_dim * 2 * lstm_layers *
                             self.bidirectional_factor, self.lstm_hidden_dim)
        ...
        self.fc3 = nn.Linear(self.anime_embedding_dim +
    ...
    def init_hidden(self, minibatch_size):
        ...
        self.hidden = (
            torch.zeros(self.lstm_layers * self.bidirectional_factor,
                        minibatch_size, self.lstm_hidden_dim).to(device),
            torch.zeros(self.lstm_layers * self.bidirectional_factor,
                        minibatch_size, self.lstm_hidden_dim).to(device)
        )
    ...
    def forward(self, X):
        ...
        target_anime_monotonic_id, past_anime_monotonic_ids, past_ratings, = X[
            :, 0:1], X[:, 1:past_anime_length + 1], X[:, past_anime_length + 1:]
        past_ratings = past_ratings.view(-1,
                                         past_anime_length, encoded_values_for_rating)
        ...
        fc1_out = self.drop1(
            F.relu(self.ln1(self.fc1(final_hidden_concat_state))))
        ...
        output_rating = (self.y_range[1]-self.y_range[0]) * \
            torch.sigmoid(final_output) + self.y_range[0]
        return output_rating


def load_pretrained_embeddings(pretrained_anime_genre_embeddings_file=pretrained_anime_genre_embeddings_file):
...
def build_learner(databunch, model, loss_func=nn.MSELoss(), pretrained_learner_fname=pretrained_learner_fname):
...
def build_model(anime_genre_embeddings, num_anime=num_anime,
                lstm_hidden_dim=lstm_hidden_dim, lstm_layers=lstm_layers, bidirectional=bidirectional,
                device=device):
    anime_genre_embeddings = torch.from_numpy(anime_genre_embeddings).float().to(device)
    model = Net(
        anime_embedding_dim=anime_genre_embeddings.shape[1], anime_embedding_vocab=num_anime,
        lstm_hidden_dim=lstm_hidden_dim, lstm_layers=lstm_layers, anime_embedding_weights=anime_genre_embeddings,
