...
anime_df = pd.read_feather(
    'model_resources/animes.feather').set_index('anime_monotonic_id')

train_df, test_df = load_dataset()
databunch = build_databunch(train_df=train_df, test_df=test_df)

all_embeddings = load_pretrained_embeddings()
model = build_model(anime_genre_embeddings=all_embeddings['anime_with_genre_embeddings'])
...
def sort_by_distance(record, anime_monotonic_id_embeddings, reverse=True):
    ...
    sorted_mask = np.argsort(
        np.vectorize(
            lambda x: cosine(
                anime_monotonic_id_embeddings[x], target_embedding)
...
def get_similar_anime_from_watch_history(previous_watch_history, anime_embeddings,
    ...
    not_watched_animes = list(
        set(anime_df.index).difference(set(watched_animes)))

    top_similar_not_watched_animes = [sorted([
        (not_watched, cosine(
            anime_embeddings[not_watched], anime_embeddings[watched]))
...
def recommendation(
    ...
    dl = DataLoader(AnimeRatingsDataset(
        user_personalised_predict_df), batch_size=batch_size, shuffle=False)
